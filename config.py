...
class ConfigForm():
    ...
    def get_login(self, usuario, senha, nome=False):
        engine = create_engine(f'postgresql://{usuario}:{senha}@35.198.41.232/postgres')
        conn = engine.connect()
    ...
    def get_results(self, nome='', matri='', endereco='', situacao='', sexo='', cpf='',
        ...
        engine = create_engine(f'postgresql://{self.config[0]}:{self.config[1]}@35.198.41.232/postgres')
        metadata = MetaData(bind=None)
        table = Table('clientes_aabb', metadata, autoload=True, autoload_with=engine)
        stmt = select([table])
